...
export async function loadHumanoidKit(playerSlot: PlayerSlotId = 'player', day?: string | null): Promise<HumanoidKit> {
  ...
  const playerSkins: Record<PlayerSlotId, ReturnType<typeof skinForSlot>> = {
    player: skinForSlot(playerCat('player'), 'player'),
    'player-f': skinForSlot(playerCat('player-f'), 'player-f'),
  };
  ...
  const kitLookIds = [...new Set<string>([...PLAYER_SLOT_IDS, ...slotIds])];
  const hairMapFiles = [...new Set(kitLookIds.map(hairFileOf))];
  const skirtMapFiles = [...new Set(kitLookIds.map(skirtFileOf))];

  const [gltf, maleMap, femaleMap, playerMapM, playerMapF, heavyMap, interceptorMap, idleRoot, runRoot, hairTexList, skirtTexList] =
    await Promise.all([
      gltfLoader.loadAsync(assetUrl(catalog.base.mesh)),
      loadMap(maleSkin),
      loadMap(femaleSkin),
      loadMap(playerSkins.player),
      loadMap(playerSkins['player-f']),
      loadMap(heavySkin),
  ...
  const playerMats: Record<PlayerSlotId, THREE.MeshPhongMaterial> = {
    player: toPhong(playerMapM),
    'player-f': toPhong(playerMapF),
  };
